...
def make_cfg_file(spc, src_epoch, src_net, tr_set):
    expt_file_str = "[ExptFiles]\n"
    expt_file_str += "data_loader: cifar100_trgt_living_vs_notliving_subset_" + str(spc) + '_' + str(tr_set)+ "\n"
    ...
    subdir_suffix = os.path.join(spc_prefix + str(spc), src_epoch_prefix + str(src_epoch), 
                                 src_prefix +str(src_net), "tr_set_" + str(tr_set))
    ...
    saved_params_str += "saved_iter: " + str(src_epoch) + "_Src_Epochs\n"
    ...
    trgt_task_params_str += "encoding_cfg: " + os.path.join(enc_dir, "softmax_encoding_35.cfg") +"\n"
    
    notes_str = "[Notes]\n"
    notes_str += "notes: Autogenerated file to tfer learn non-living CIFAR100 from living CIFAR100\n\n"
...
def make_cfg_files_and_dirs():
    root_dir = os.path.join('..', expt_dir,
                            ...
                            expt_datasets,
                            expt_arch, 'tfer_nets' )
    for curr in product(spc_list, src_net_list,
                        tfer_tr_set_list):
        spc, src_net, tr_set = curr
        src_net_root_dir =  os.path.join('..', res_dir,
                                         expt_class,
                                         expt_datasets,
                                         expt_arch,
                                         src_nets_dir, src_net)
        src_net_expts = os.listdir(src_net_root_dir)
        for curr_src_net_expt in src_net_expts:
            saved_nets = [x[:-3]
            ...
            for curr_src_net in saved_nets:
                src_epoch = curr_src_net[:-3].split['_'][0]
                ...
                if len(cfg_files) == 0:
                   ...
                   version = max(cfg_files) + 1
                cfg_file_name = "tfer_net_" + str(version) + ".cfg"
